    ...
    pub fn eval_expr(&mut self, expr: Expression) -> Result<RuntimeVal, String> {
        ...
        match expr {
            ...
            Expression::ErrorRef(name_val) => {
                let name = crate::grammar::struct_name(&name_val).to_string();
            ...
            Expression::StructInit(name, _, fields, _) => {
                ...
                for f in fields {
                    ...
                    data.insert(crate::grammar::field_name(&f.name).to_string(), val);
                }
                // 2. Return Struct Value
                Ok(RuntimeVal::Struct(
                    crate::grammar::struct_name(&name).to_string(),
                    data,
                ))
            }

            Expression::FieldAccess(target, _, field) => {
                let field_name = crate::grammar::field_name(&field);
                ...
                match val {
                    ...
                    RuntimeVal::Module(exports, _) => exports
                        .get(field_name)
                        .cloned()
                        .ok_or_else(|| format!("Export '{}' not found in module", field_name)),
                        ...
                        match &*guard {
                            RuntimeVal::Struct(_, fields) => {
                                fields.get(field_name).cloned().ok_or_else(|| {
                                    format!("Field '{}' not found in struct pointer", field_name)
                                })
                            }
                            _ => Err(format!(
                    ...
                    RuntimeVal::AdrHandle(Some(ptr)) => {
                        ...
                        match &*guard {
                            RuntimeVal::Struct(_, fields) => {
                                fields.get(field_name).cloned().ok_or_else(|| {
                                    format!("Field '{}' not found in address handle", field_name)
                                })
                            }
                            _ => Err(format!(
            ...
            Expression::Ref(_, target) => {
                ...
                if let Expression::Variable(v) = &*target {
                    let name = crate::grammar::variable_name(v);
                    if let Some(stmt) = self.functions.get(name) {
                        if let Statement::FunctionDef(def) = stmt {
                            ...
                            return Ok(RuntimeVal::FunctionRef(name.to_string()));
                        }
                    }
                }
            ...
            Expression::Call(func_var, _, args, _) => {
                ...
                let result = (|| {
                    ...
                    let (func_stmt, func_debug_name, host_module_hint) = match *func_var {
                        Expression::Variable(v) => {
                            let name = crate::grammar::variable_name(&v);
                            let mut f = self.functions.get(name).cloned();
                            let mut debug_name = name.to_string();
                            if f.is_none() {
                        ...
                        Expression::FieldAccess(target, _, field) => {
                            ...
                            let module_hint = match &*target {
                                Expression::Variable(v) => {
                                    Some(crate::grammar::variable_name(v).to_string())
                                }
                                _ => None,
                            };
                            let val = self.eval_expr(*target)?;
                            if let RuntimeVal::Module(_, funcs) = &val {
                                let field_name = crate::grammar::field_name(&field);
                                let f = funcs.get(field_name).cloned();
                    ...
                    if let Statement::FunctionDef(def) = func_stmt {
                        ...
                        if pure_kw.is_some() {
                            ...
                            for arg_expr in &args {
                                ...
                                if let Expression::Variable(v) = current
                                    && let Some(entry) =
                                        self.env.get(crate::grammar::variable_name(v))
                                    && entry.is_mutable
                                {
                                    return Err(format!(
                        ...
                        for (i, param) in params.clone().into_iter().enumerate() {
                            let param_name = crate::grammar::param_name(&param).to_string();
                            fn_env.insert(
                                param_name,
                        ...
                        let def_name = crate::grammar::function_name(&def.name).to_string();
                        ...
                        match self.host_mode {
                            ...
                            HostMode::Execute => {
                                let handler = self
                                    .host_registry
                                    .get(&host_module_name, &def_name)
                                    .ok_or_else(|| {
                                        format!(
                                            "Interpreter Error: Host function '{}.{}' is not registered.",
                                            host_module_name, def_name
                                        )
                                    })?;
