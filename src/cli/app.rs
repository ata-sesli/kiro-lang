...
use crate::build_manager::{BuildManager, BuildRequirements, GENERATED_BUILD_DIR};
use crate::compiler;
use crate::eir::{LowerError, LowerErrorKind, VerifyErrorKind};
use crate::errors::{self, KiroError, emit_error};
use crate::formatter;
use crate::host_generator::{self, HostGenOptions};
use crate::interpreter::HostMode;
use crate::interpreter::eir_runtime::{EirRuntime, EirRuntimeError, EirRuntimeErrorKind};
use crate::project;
use crate::test_runner;
use crate::{StdAssets, canonical_std_module_name, is_reserved_std_module_name, std_asset_path};
...
fn run_interpreter(analysis: &analysis::AnalysisResult) -> bool {
    let program = match crate::eir::lower_program(&analysis.hir) {
        Ok(program) => program,
        Err(error) => {
...
fn run_compiler(
    ...
    let program = crate::eir::lower_program(&analysis.hir)
